...
export default function ExplorerTransactionSearch() {
  const [txInput, setTxInput] = useState("");
  const router = useRouter();

  const handleSearch = () => {
    const trxId = txInput.trim();
    if (trxId) router.push(`/explorer/transaction/${trxId}`);
  };
      ...
      >
        <CardBody className="flex flex-col items-center justify-center py-16 gap-3">
          <div className="p-4 rounded-2xl bg-primary/5">
            <ArrowRightLeft size={40} className="text-primary/30" />
          </div>
          <p className="text-default-500 dark:text-default-400 text-sm text-center max-w-sm">
            Enter a transaction hash to view its details, operations, and
            signatures.
          </p>
        </CardBody>
      </Card>
