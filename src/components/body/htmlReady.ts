...
import linksRe, { any as linksAny } from '@/libs/utils/Links';
import * as Phishing from '@/libs/utils/Phishing';

export const getPhishingWarningMessage = () => 'Alert: recognized as phishnig link';
...
function linkify(content, mutate, hashtags, usertags, images, links) {
    ...
    content = content.replace(linksAny('gi'), ln => {
        ...
        if (Phishing.looksPhishy(ln))
            return `<div title='${getPhishingWarningMessage()}' class='phishy'>${ln
                }</div>`;
