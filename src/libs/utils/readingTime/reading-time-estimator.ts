 ...
 */
const parseWords = (data: string): string[] => {
    if (data && data?.match) {
        return data?.match(/[\w|\d|\s|,|.|\u00C0-\u024F|\u4E00-\u9FA5|\u3041-\u309F]+/giu) ?? [];
    } else return []
}
/**
     ...
     */
const getNumberOfWords = (data: string): number =>
 ...
 */
const isLessThanAMinute = (minutes: number): boolean => minutes < 1 + Number.EPSILON;
 ...
 */
const getLocale = (minutes: number, locale: SupportedLanguages = 'en'): string =>
