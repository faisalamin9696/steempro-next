...
export const makeOptions = (postObj: {
  ...
  const operationType = postObj.payoutType.payout;
...
export const makeJsonMetadata = (
  meta: { links: string[]; image: string[]; users: string[] },
  tags: string[]
) =>
  Object.assign({}, meta, {
    tags,
...
export const extractMetadata = (
  body: string
): { links: string[]; image: string[]; users: string[] } => {
  const urlReg =
    /(\b(https?|ftp):\/\/[A-Z0-9+&@#/%?=~_|!:,.;-]*[-A-Z0-9+&@#/%=~_|])/gim;
  const userReg = /(^|\s)(@[a-z][-.a-z\d]+[a-z\d])/gim;
  const imgReg = /(https?:\/\/.*\.(?:png|jpg|jpeg|gif))/gim;

  const out: { links: string[]; image: string[]; users: string[] } = {
    links: [],
    image: [],
    users: [],
  };
...
export const validateCommentBody = (
  ...
  if (new Blob([body]).size >= maxLength - 256) {
...
export const validateCommunityAccount = (tag?: string): boolean => {
  if (!tag) return false;
